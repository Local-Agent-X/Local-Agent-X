...
| `public/` | core only via `self_edit` | **no — COMMITTED** | Shipped UI chrome (app.html, settings.html, sidebar). Don't add user pages here. |
...
- `public/` is committed. Any file added here ships to every user of SAX on every machine. Only the core engine chrome belongs here.
...
- **Agent-facing tools belong in `tools.ts` registry.** Model can only see what's registered. Register → policy rule → done.

---

## Security posture

- **Ari Kernel is in-process** (see `src/ari-kernel.ts`). Every tool call runs through it. Don't add tool execution paths that bypass.
- **Default-deny policy** — new tools need an explicit `allow-<name>` rule in `src/tool-policy.ts`.
