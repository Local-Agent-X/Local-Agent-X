...
| Path | Who touches | Rules |
|---|---|---|
| `src/` | `self_edit` only (protected for direct edit) | Core engine. TypeScript. One responsibility per module. |
| `config/` | agent or user | Agent-editable behavior (prompt, tools allowlist). Hot-reloads. |
| `public/` | core only via `self_edit` | UI assets. HTML/JS/CSS. |
| `workspace/` | agent or user | User files, built apps, scratch. |
| `packages/arikernel/` | hands off unless fixing a specific Ari bug | Vendored security layer. |
| `dist/` | build output — never hand-edit | Regenerated by `npm run build`. |
