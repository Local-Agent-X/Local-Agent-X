...
| **Browser backends + secret guards** | `src/browser/instance.ts`, `src/browser/in-app-backend.ts`, `src/browser/runtime.ts`, `src/browser/secret-ops.ts` | `browserMode` defaults to `in-app`: an embedded WebContentsView the user can watch and co-drive (`desktop/src/browser-views.ts`), one per (session, profile) on a hardened partition. `resolveBrowserRoute()` in `instance.ts` is the one fallback matrix — a non-`in-app` mode, `LAX_BROWSER_HEADLESS=1`, or an absent desktop bridge routes to the external-Chrome CDP `BrowserManager` instead, and says which and why. `isolated` / `continuity` / `advanced-shared` select external Chrome with per-session, persistent, or shared identity. Secret fill/capture run on **both** backends via `SecretBrowserOps` and enforce credential-field selectors, exact origin binding, and first-use approval/pre-blessing. Every mode uses an agent-owned profile, never the user's own browser profile. |
...
| **Exfil / threat engine** | `src/threat/` (`threat-engine.ts`) | Per-turn temporal threat scoring + data-flow exfil blocks (`scoring.ts`), tool-chain risk (`tool-chain.ts`), trust ledger, canaries, hash-chained audit trail. Classifications feed the policy gate. |
| **Data lineage / taint tracking** | `src/data-lineage/` (`index.ts`) | Session-scoped taint registry complementing the threat engine's regex checks: data read from a sensitive source (`paths.ts`) is fingerprinted (`fingerprint.ts`) and tainted (`taint.ts`); if that data later reaches an egress channel it is blocked by CALL SEQUENCE even after transforms (base64/chunk/`bash`), because the LLM can't "un-see" a sensitive source it already ingested. |
...
| **Persona / cognition** | `src/cognition/` | The persona constellation (emotional memory, narrative memory, correction learning, anticipatory care, cross-session learning, …), consolidated from ~18 root modules. Distinct from `src/memory/cognitive/`. Shared JSON persistence helper: `src/util/json-store.ts`. |
| **Self-learning protocols** | `src/cognition/cross-session-learning/` + `src/protocols/learned-*.ts`, `docs/learned-protocols.md` | Mines proven workflows into versioned, outcome-scored learned `SKILL.md` protocols. Seams: evidence at `src/canonical-loop/turn-loop/record-outcome.ts`, reconcile driver at `src/orchestrator/signals-meta.ts` (every 5th message), capability-envelope gate at `src/tool-execution/learned-protocol-envelope.ts`, surfacing at `src/agent-request/prepare-request/build-context.ts`, HTTP at `src/routes/memory-learning.ts`. `learningMode` (`assisted` default / `autonomous`) is a protected setting; a learned protocol only *restricts* capability, never expands it. |
...
| **Session / context state** | `src/session/` (`router.ts`), `src/context/` (`system-prompt-builder.ts`) | Token-budget auto-compaction is a separate live module — `src/context-manager/` (single budget/compaction policy table: `compaction-policy.ts`), driven by the loop's `turn-loop/compact-history.ts`. Run/template/project/issue persistence: `src/agent-store/`. |
...
| **Computer control (mouse/kbd)** | `src/computer-control.ts`, `src/tools/input-tools.ts` (the `computer` tool) | nut.js driver in `src/tools/input-driver.ts`; off by default (`enableComputerControl`), gated in `src/tool-execution/pre-dispatch.ts`, typed text classed as egress. |
