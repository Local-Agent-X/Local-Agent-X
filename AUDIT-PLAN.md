...
**Operating model.** Claude Code (parent session) orchestrates. Each chunk runs in a **fresh Claude Code session** that Peter spawns by pasting the chunk briefing (below). The fresh session inherits no context — the briefing is self-contained. After each chunk, parent session reviews the diff, runs the smoke test, and commits.
