...
- `autonomous`: under the protected `learningMode`, draft, activate, select, refine, demote, roll back, and archive learned protocols without per-protocol permission prompts.
...
- Authenticated learning reads remain available to the management UI. Manual HTTP lifecycle mutations require the operator role; autonomous reconciliation intentionally performs the same lifecycle work in process without routing through a manual permission prompt.

## Current trust and evidence boundary

- System-managed learned protocols live in the machine-local `~/.lax/protocols/learned/` trust root. Lifecycle-marker directories left in `workspace/protocols/imported/` by the legacy layout are ignored rather than treated as managed protocols.
- Ordinary user-authored `workspace/protocols/imported/` protocols remain supported. Managed protocols load from the separate trust root after workspace imports, so a self-consistent workspace forgery cannot shadow or masquerade as a system-managed learned protocol. Within the trusted store, containment, symlink, metadata, and content-hash checks reject inconsistent or redirected records; this is not an HMAC or signature scheme.
- Externally influenced, sensitive-tainted, native-external, and unknown-provenance outcomes are automatically excluded from both workflow evidence and learned-protocol effectiveness evidence. The originating task is not interrupted and no new approval is requested; it simply does not train later behavior.
- Candidate evidence, effectiveness receipts, and system-managed learned protocols are local installation state. The system rebuilds missing managed protocol material from its canonical local learning evidence, but it does not claim trusted cross-machine synchronization of learned state. Workspace user imports remain a separate user-controlled input.
- Persistence interruption and restart repair are exercised with deterministic injected write failures on Windows and POSIX rather than filesystem permission-bit behavior.
...
- C4 verification: durable machine-local learned-protocol storage and candidate drafting passed 14 focused tests, TypeScript, restart, integrity, traversal, idempotency, and adversarial checks. Only outcome-proven workflow candidates draft; drafts remain undiscoverable; ordered tool identities, confidence, and immutable evidence provenance are preserved; unchanged evidence does not create versions and stronger evidence does.
...
- C5 verification: operation-scoped learned-protocol envelopes passed 28 focused tests, TypeScript, source hygiene, restart persistence, conflict detection, per-tool provenance revalidation, and independent refutation. Learned allowlists only narrow the normal authorization path; forged operation IDs, stale versions, modified bodies, and removed protocols fail closed while ordinary imported protocols remain compatible. Subsequent trust-root hardening separated managed protocols from agent-writable workspace imports and proved that a complete forged workspace lifecycle record cannot become managed, selected, or dispatched.
- C6 verification: the assisted/autonomous coordinator, immutable-evidence repair, and automatic learned-workflow selector passed 54 combined focused tests, TypeScript, source hygiene, and independent refutation. Assisted mode drafts and waits; autonomous mode activates, refines, rolls back, and archives quietly without per-protocol prompts; every-message selection admits only verified active candidate-linked protocols and injects a short canonical loading hint without bodies, evidence, or permissions. Manual HTTP mutations remain operator-only.
...
- C7 verification: the terminal feedback bridge passed 120 focused and forced-path checks across normal completion, iteration checkpoints, token and deadline exhaustion, adapter and worker exceptions, cancellation, restart recovery, and state mismatch quarantine. Provenance is captured before terminal cleanup, pending receipts precede durable terminal writes, committed receipts follow them, eligible ordinary work still feeds candidate mining, and replay preserves original timestamps. Externally influenced, sensitive-tainted, native-external, and unknown-provenance outcomes are excluded automatically from workflow and effectiveness evidence without interrupting their tasks.
...
- C12 verification: the strict production-seam test passed three consecutive runs and then passed again after an added false-positive rollback assertion. It proves ordinary canonical outcomes through mining, assisted drafting, autonomous activation, every-message selection, trusted operation provenance, narrowing-only capability enforcement, exact effectiveness receipts, stronger refinement, mode-independent rollback, stale-envelope shutdown, immutable prior versions, cancellation exclusion, and persistence-interrupted restart recovery. Restart recovery is now tested with deterministic injected failures on Windows and POSIX rather than POSIX permission bits. The 20-file campaign matrix passed 131 tests; the final changed-test matrix passed all 22 files and 182 tests.
- Independent final refutation: a separate skeptic returned SHIP after rerunning C12 (3 tests) and a targeted C5-C8 production-wiring matrix (8 files and 71 tests). It confirmed real model-facing selection, forged-operation-ID replacement, narrowing-only authority, terminal persistence ordering, cancellation exclusion, activation-bounded chronological rollback, safety-rejected-version suppression, and fail-closed integrity/path checks inside the machine-local managed trust root.
...
- C4: machine-local system-managed learned-protocol storage plus deterministic, outcome-proven candidate drafting with active-only discovery and exact tool-order provenance; ordinary workspace user imports remain separate and supported.
...
- C7: restart-repairable, envelope-attributed effectiveness receipts with pending-before-terminal and commit-after-terminal ordering, while preserving eligible ordinary candidate mining and excluding cancellation or externally influenced, sensitive, native-external, and unknown-provenance outcomes.
