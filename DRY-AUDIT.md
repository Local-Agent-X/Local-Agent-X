...
- **User facts in memory vs system prompt** — System prompt is intentionally generic; memory stores user facts via auto-extract. Working as designed (no `Peter`/`NutriShop`/`ScanProgress` hardcoded in `config/system-prompt.md`).
