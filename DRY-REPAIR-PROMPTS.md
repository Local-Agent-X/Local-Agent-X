...
**Branch protocol** (revised 2026-05-13 after the Group A worktree collision and Peter's "everything to main" directive): each session commits directly to `main`. Only fan out branches/worktrees when multiple sessions are running concurrently and would otherwise share an uncommitted worktree. For solo or sequential dispatches, work and commit on `main`. Do NOT push (Peter pushes).
...
Commit on main (only after Peter confirms 3B.1 soak passed). Do NOT push.
