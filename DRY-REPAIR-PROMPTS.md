...
Each section below is a **complete, self-contained prompt** to copy-paste into a fresh Claude Code session. Sessions report back to `docs/dry-repair-reports/<task-id>.md` and Alex feeds the report to the orchestrator session.
...
3. Migration audit mode: gate the new strict block behind
   ...
   This lets Alex do one soak session before flipping enforcement.
...
1. Update SECURITY.md — replace "[TBD - add security contact email]"
   with petermanrique101@gmail.com (Alex's email, confirmed).
...
Include in the report: a soak plan (what Alex should listen for,
...
and let Alex soak it for a day before merging.
...
**Wait for: 3B.1 merged AND Alex signs off on soak (no latency regression).**

```
You are completing one task in a coordinated multi-session DRY repair effort.
Working dir: c:\Users\manri\local-agent-x

Prereq: 3B.1 merged + Alex has confirmed soak passed (warm-path latency
...
2. In gpu-session.ts: same — including the more aggressive CLAUSE_BREAK
   ...
   isn't already in open-voice, FLAG IT and stop. Alex will decide
...
no longer has a working legacy path to fall back to. Make sure Alex
...
- If the three scripts currently DO DIFFERENT THINGS (not just
  ...
  Alex needs to decide what the right behavior is, not you.
...
3. Alex reviews the diff + report; merges into main (or branch) at his cadence.
4. Alex pastes (or shares the path of) the report back to the orchestrator session — that's me.
5. Orchestrator updates its todo list, decides what becomes safe to dispatch next, hands the next prompt.

If a session reports `blocked` or surfaces a surprise that affects another task's prompt, the orchestrator session adjusts and re-emits the affected prompt before Alex dispatches it.
