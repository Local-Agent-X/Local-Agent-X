...
Shell tools default to the **guarded** kernel cage on macOS and Linux: credential paths are denied while normal development network access remains available. Where that cage is unavailable (including Windows), the effective mode falls back visibly to `host`; unattended shell use then requires an explicit host acknowledgement. Select stricter `seatbelt`, `bwrap`, or `docker` isolation in Settings or with `LAX_SANDBOX`. See [docs/ari-kernel.md](docs/ari-kernel.md) for the in-process policy layer and [SECURITY.md](SECURITY.md) for deployment guidance.
