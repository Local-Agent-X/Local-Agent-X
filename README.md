...
A self-hosted personal agent platform. It runs on your machine, and the server binds to `127.0.0.1` only. LAX-owned provider credentials and secrets are encrypted at rest; chat sessions, memory, uploads, and audit logs are stored locally by default but are not all encrypted by LAX, so use full-disk encryption for that boundary. Agent Sync can mirror configured memory, sessions, sanitized configuration/sidebar state, and workspace data to a user-configured Git remote; use a private repository. Prompts and attached context go to the selected LLM provider; browser, web, email, connector, MCP, sync, and other external tools can send data to their configured services when invoked. A local Ollama model keeps model inference on-device, but it does not disable those separately configured outbound capabilities.

Shell tools default to the **guarded** kernel cage on macOS and Linux: credential paths are denied while normal development network access remains available. Where that cage is unavailable (including Windows), the effective mode falls back visibly to `host`; unattended shell use then requires an explicit host acknowledgement. Settings exposes guarded, unconfined host, and network-isolated Docker choices. The stricter network-denying `seatbelt`/`bwrap` profiles are selected through `LAX_SANDBOX` or config. See [docs/ari-kernel.md](docs/ari-kernel.md) for the in-process policy layer and [SECURITY.md](SECURITY.md) for deployment guidance.

Beyond chat, it ships voice (push-to-talk and full-duplex), scheduled missions on cron, a tool-executor with a default-deny policy, and a workspace where the agent builds and serves its own small apps. The runtime UI is a single HTTP server you open in any browser. Self-modification routes through a `config/` directory that hot-reloads and through connector manifests in the data directory (an external API becomes `/api/connectors/<name>/...` without touching core code); source-level changes go through a `self_edit` tool that is off by default and unlocked by the `developer_mode` setting. Platform updates are validated before they land: every update is built, booted, and smoke-tested in an isolated sandbox before it can replace the running install.
...
**Reading the code?** [ARCHITECTURE.md](ARCHITECTURE.md) is the current code map — every major subsystem to its canonical file, with superseded/dead directories flagged so you don't trace into them. Start there; its companion [docs/codebase-map.md](docs/codebase-map.md) is auto-generated (live importer counts, size tiers, god files, and the genuinely-dead dirs) and the build keeps it fresh. Historical design and migration records under `docs/` are labeled as such and defer to these runtime owners.

Three lanes for change: **runtime state** flips through HTTP endpoints (settings, provider, theme — never edit files for live state), **self-modification** goes into the agent-editable `config/` directory which hot-reloads, and **external sites** flow through the `browser` tool rather than raw fetch. Every tool call runs through the `src/tool-execution/` pipeline and the in-process Ari Kernel security layer; new tools need an explicit allow rule in the per-tool policy table (`src/tool-policy/tool-policies.data.ts`). The architectural rules are non-negotiable and live in [AGENTS.md](AGENTS.md) — read that before touching the codebase.
