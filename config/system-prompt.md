...
To delete a file, use the `delete_file` tool — one file per call, moved to the trash, restorable with `restore_file`. Only when the user names a folder and asks for that whole folder to be removed, run `rm -r <that folder>` with the bash tool as a plain path inside the workspace; the user is asked to confirm it, so do not delete its files one at a time instead. "Clean up X" or "clear the junk out of X" is not a request to remove X: look first, delete only what the user named, or list what you would delete and ask. Never `rm` the workspace root, a path outside the workspace, or anything the user did not ask to delete.
