...
- **Dataset:** LongMemEval-S (`longmemeval_s_cleaned.json`), all 500 questions. Not committed — download the cleaned dataset and place it at `workspace/benchmarks/longmemeval/longmemeval_s_cleaned.json` (the `workspace/` tree is gitignored) before running.
...
|---|---|---|---|---|---|---|
| **thenlper/gte-large** | 1024 | **97.2%** | 486/500 | sentence-transformers | yes (local) | **+0.6 — beats** |
...
All numbers are zero-API, fully local. gte-large reproduced at 97.0% on a clean re-run (run-to-run variance of ~1 question); peak observed 97.2%.
...
2. **Better embedder → we beat them.** Swap in stronger free local embedders and we pull ahead: **mxbai-embed-large 97.0%**, **gte-large 97.2%** — both above MemPalace's 96.6%.
...
This is deliberately **not** our single highest score (gte-large's 97.2% is ~0.2 higher). It is the **highest score achievable with zero friction**:
...
For users who want the absolute ceiling and don't mind running the sentence-transformers server, **gte-large reaches 97.2%** (97.0% on the confirming re-run) — our highest measured R@5. The sidecar is [`scripts/embed-server.py`](../../scripts/embed-server.py); point the embedder at it via `--emb-url http://127.0.0.1:11435`. Proof: [`raw-results/gte-large-r5-confirm.log`](raw-results/gte-large-r5-confirm.log).
...
# Ceiling (gte-large, sidecar) — 97.2%
