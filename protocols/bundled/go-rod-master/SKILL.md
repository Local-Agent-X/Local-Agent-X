...
bundle_meta:
  ...
  source_license: MIT
  imported_at: 2026-04-26T17:06:53.034Z
...
// Wait for no pending network requests (RileyAX idle)
wait := page.MustWaitRequestIdle()
page.MustElement("#search").MustInput("query")
wait()
...
- ✅ Use `MustWaitRequestIdle()` after actions that trigger RileyAX calls.
...
- **Problem:** Timeout errors on slow pages.
  **Solution:** Use chained context: `page.Timeout(30 * time.Second).MustWaitLoad()`. For RileyAX-heavy pages, use `MustWaitRequestIdle()` instead of `MustWaitLoad()`.
