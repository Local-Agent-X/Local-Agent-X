...
export function createWorktree(agentId: string): { path: string; branch: string } | null {
  try {
    ...
    console.log(`[worktree] Created ${wtPath} on branch ${branch} (base: ${baseBranch})`);
    return { path: wtPath, branch };
  } catch (e) {
    console.warn(`[worktree] Failed to create: ${(e as Error).message}`);
    return null;
  }
}
...
export function mergeWorktree(agentId: string): { merged: boolean; files: number; error?: string } {
  ...
  try {
    ...
    try {
      ...
      console.log(`[worktree] Merged ${fileCount} files from ${agentId} into ${wt.baseBranch}`);
      wt.mergedSuccessfully = true;
      cleanupWorktree(agentId);
      return { merged: true, files: fileCount };
    } catch {
      git("merge --abort", wt.repoRoot);
      console.warn(`[worktree] Merge conflict for ${agentId} — changes preserved on branch ${wt.branch}`);
    ...
    console.warn(`[worktree] Merge failed: ${(e as Error).message}`);
    cleanupWorktree(agentId);
...
export function cleanupWorktree(agentId: string): void {
  ...
  if (wt.mergedSuccessfully) {
    ...
    console.log(`[worktree] Preserving branch ${wt.branch} (unmerged changes)`);
  }

  activeWorktrees.delete(agentId);
  console.log(`[worktree] Cleaned up ${agentId}`);
}
