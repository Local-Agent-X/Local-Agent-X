...
import { ensureDurableBridge, reconcileRecoveredAsk, recordDurableRequest, recordDurableResolve } from "./approval-durable-record.js";
...
class ApprovalManager {
  ...
  async requestApprovalDetailed(opts: {
    ...
    // Then reconcile against a crash-surviving pendingApproval column
    // (approval-durable-record.ts reconcileRecoveredAsk): a decision the user
    // recorded post-restart for THIS exact ask is applied immediately — no
    // new card — and a plain matching survivor lends the re-ask its original
    // window instead of restarting the 5-min budget from zero.
    let carriedRequestedAt: number | null = null;
    if (opts.opId) {
      await ensureDurableBridge();
      const rec = reconcileRecoveredAsk(
        opts.opId, opts.toolName, argsPreview, (id) => this.pending.has(id), APPROVAL_TIMEOUT_MS,
      );
      if (rec.recordedDecision) {
        return rec.recordedDecision.approved
          ? { approved: true }
          : { approved: false, reason: "declined" };
      }
      carriedRequestedAt = rec.carriedRequestedAt;
    }
