 ...
 *
 * Why this exists: `run_build_plan` runs for hours. LAX gets
...
export const ORCHESTRATOR_STATE_FILENAME = ".lax-build-run.json";
/** Pre-rename filename — read (and cleared) for runs started before the rename. */
const LEGACY_STATE_FILENAME = ".primal-orchestrator-state.json";
...
export function exists(projectDir: string): boolean {
  return existsSync(statePath(projectDir)) || existsSync(join(projectDir, LEGACY_STATE_FILENAME));
}
...
export function read(projectDir: string): OrchestratorState | null {
  let p = statePath(projectDir);
  if (!existsSync(p)) p = join(projectDir, LEGACY_STATE_FILENAME);
