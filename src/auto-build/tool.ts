 ...
 *
 * Gated behind the LAX_AUTO_BUILD_ENABLED env flag — default off.
...
/** Pre-rename flag name — honored when the new one is unset. */
...
export const runBuildPlanTool: ToolDefinition = {
  name: "run_build_plan",
  description:
  ...
  async execute(args, signal): Promise<ToolResult> {
    ...
    if (!projectDir) {
      return { content: "run_build_plan requires 'project_dir' (bare project name like 'petbook', or an absolute path).", isError: true };
    }
    ...
    // .lax-build-run.json — build_plan_resume picks it up.
