...
/** Resolve the workspace/downloads directory and ensure it exists. */
...
// Track recent downloads so the agent can be told where the file landed
// after a navigate/click that triggered one.
...
export class BrowserManager {
  ...
  private adoptPage(p: Page): Page {
    ...
    // Download handler — without this, Playwright aborts navigation with
    // "Download is starting" and the file lands nowhere. Save to
    // workspace/downloads/ so the file is reachable from agent tools
    // (read, view_image, edit) AND syncs across machines via the workspace
    // git sync. Users are expected to clean up huge files themselves
    // (>100MB will break GitHub sync — agent should warn on those).
