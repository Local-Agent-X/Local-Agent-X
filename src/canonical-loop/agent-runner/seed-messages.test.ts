// Mirror of chat-runner/seed-messages.test.ts for the AGENT-runner seeding
// seam: a caption-less photo row round-tripped through history as
// {content:"", images:[{name, url:"/uploads/<f>"}]} used to fall to this
// seeder's emptiness filter — a spawned agent (autopilot, delegation ack,
// etc.) lost sight of an image it was shown a turn ago. The fix consumes the
// SAME bounded-revival policy module the chat-runner uses
// (../history-image-revival.ts) — this suite proves the shared policy holds
// on the agent path through the REAL seams: seedOpMessages → buildTurnInput
// → canonicalToTransport → Anthropic convertMessages / Gemini toGeminiContents.
...
const SHOT_NAME = "agentseed-shot.png";

let opSeq = 0;
let opId = "";
beforeEach(() => {
  opId = `op_agent_seed_images_${opSeq++}`;
  mkdirSync(uploadsDir(), { recursive: true });
  writeFileSync(join(uploadsDir(), SHOT_NAME), Buffer.from(PNG_B64, "base64"));
});

function op(): Op {
  return { id: opId, type: "agent_turn", task: "look again", lane: "background" } as unknown as Op;
}

// The exact history row a caption-less photo send round-trips to
// ({name, url} only — filePath stripped by the UI projection).
...
describe("agent-runner seedOpMessages — caption-less photo rows in seeded history", () => {
  it("survives seeding with filePath restored from the /uploads url, and reaches both wires as a real image", async () => {
    seedOpMessages(
      opId,
      [photoRow(SHOT_NAME), { role: "assistant", content: "A tidy dashboard." }],
  ...
  it("bounded revival: 7 unique images → only the 6 most recent get bytes; a repeated url revives only at its LAST occurrence", async () => {
    ...
    const names = ["A", "B", "C", "D", "E", "F", "G"].map(n => `agentcap-${n}.png`);
