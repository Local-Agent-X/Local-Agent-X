...
describe("container connectivity identity", () => {
  it("rejects a remote hostname as a host gateway", () => {
    ...
    expect(() => validatedHostGateway()).toThrow("not host-local");
  });

  it("accepts only a private literal or the Docker host alias", () => {
    ...
    expect(validatedHostGateway()).toBe("host.docker.internal");
  });
