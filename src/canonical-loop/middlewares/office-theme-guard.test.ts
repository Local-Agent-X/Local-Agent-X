...
describe("office-theme-guard", () => {
  it("strips an uninvited theme from an office tool call (string args)", () => {
    const tc = { tool: "presentation_from_outline", args: JSON.stringify({ file_path: "a.pptx", outline: "# X", theme: '{"colors":{"accent":"#C41E3A"}}' }) };
  ...
  it("strips from object args too", () => {
    const tc = { tool: "document_create", args: { file_path: "a.docx", content: "x", theme: "{}" } };
  ...
  it("keeps the theme when the user asked for a look", () => {
    const tc = { tool: "presentation_create", args: { file_path: "a.pptx", slides: "[]", theme: '{"colors":{"accent":"#C41E3A"}}' } };
  ...
  it("keeps brand-keyword requests", () => {
    const tc = { tool: "pdf_create", args: { file_path: "a.pdf", content: "x", theme: "{}" } };
