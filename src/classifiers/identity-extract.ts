...
Output JSON with these fields (omit or set to null when not stated):
{
  ...
  "ongoing_state": <Array<string> | null>,  // any DURABLE PRESENT-TENSE fact about the user — something that should still be true tomorrow. The catch-all for "I'm currently X" / "I'm doing X" / "I have X" / "I own X" / "I live in a Y" patterns that aren't already captured by location/employer/role/family/relationships/preferences/affinities/events. Examples across categories: medications/health ("I'm taking <med>" → ["User is currently taking <med>"], "I have type 2 diabetes" → ["User has type 2 diabetes"]); diet/fitness ("I'm on keto" → ["User is on a keto diet"], "I work out 3x a week" → ["User works out 3x a week"]); work/projects ("I'm building a CRM" → ["User is currently building a CRM"], "I'm studying for the bar exam" → ["User is studying for the bar exam"]); learning ("I'm learning Spanish" → ["User is learning Spanish"]); possessions ("I drive a 2022 pickup" → ["User drives a 2022 Toyota pickup"], "I have two cats" → ["User has two cats"]); habits/lifestyle ("I'm a night owl" → ["User is a night owl"], "I'm vegetarian" → ["User is vegetarian"]); current situation ("I'm staying with my parents" → ["User is currently staying with their parents"]). NOT category-specific — if a statement is durable + present-tense + about the user and doesn't fit another field, it goes here. Phrase third-person, ≤180 chars. Each statement in its own array entry. EACH new addition is a separate entry — "I'm also taking X" after a prior med is a SECOND entry, not an update.
...
export async function extractIdentityFactsWithLLM(
  ...
  return classifyJson<IdentityFacts>({
    ...
    validate: (parsed) => {
      ...
      if (Array.isArray(p.ongoing_state)) {
