...
export class MemoryIndex {
  ...
  private migrateSchema(fromVersion: number): void {
    ...
    this.db.transaction(() => {
      ...
      // "Alex decided to ship the mobile release" → subject=peter, predicate=decided, object=mobile-release
  ...
  retain(text: string, sourceFile: string, sourceLine = 0): RetainedFact[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      try {
        ...
        // e.g. "Alex decided to ship mobile" → (peter, decided, ship-mobile)
   ...
   *   1. In-sentence SPO: "Alex decided to ship X" → (peter, decided, x)
