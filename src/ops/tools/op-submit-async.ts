...
export const opSubmitAsyncTool: ToolDefinition = {
  ...
  description:
    "DELEGATION PICKER: `op_submit_async` = ONE background task you want a worker to finish (research, a build, an integration). `operation_start` = a multi-PHASE goal needing decomposition + checkpoints. `agent_spawn` = hand work to a NAMED catalog agent/role. `build_app`/`primal_run_build_plan` = build a whole app from a spec. " +
