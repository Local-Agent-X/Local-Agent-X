...
export const opSubmitAsyncTool: ToolDefinition = {
  ...
  description:
    "DELEGATION PICKER: `op_submit_async` = a background task you want a worker to finish (research, a build, an integration, or a multi-step goal — the worker plans and checkpoints its own steps). `agent_spawn` = hand work to a NAMED catalog agent/role. `build_app`/`primal_run_build_plan` = build a whole app from a spec. " +
