...
import { scanForSecrets, redactSecrets, decodedPayloadViews } from "./secret-scanner.js";
...
// ── Budget starvation: one expensive run must not blind the rest of the scan ──
//
// Pre-existing structural weakness (documented in the f8c0ceb8 revert): every run
// and every layer of a whole scan drew on ONE MAX_DECODED_BUDGET counter, and each
// consumer did `if (budget.remaining <= 0) break`. So one expensive early run
// walked the counter to zero and every LATER run in the document was skipped
// entirely — the catalog pass, the known-value pass that drives the egress BLOCK,
// and the taint-overlap views were all defeated in one stroke, silently, with
// clean=true. The fix is two bounds (a per-run slice of the scan budget) plus
// `continue` instead of `break`, plus a `truncated` signal so a scan that gave up
// can never be mistaken for a scan that came back clean.

/**
 * A deliberately expensive encoded run of roughly `outerLen` chars: `layers`
 * rounds of base64 over a base64-charset seed, so every peel layer reveals
 * another inner run and the decode engine keeps materializing views. This is the
 * shape the adversarial reviewer used; it needs no decode-to-backslash quine.
 * The seed is sized backwards from `outerLen` so the DOCUMENT stays the same
 * size across depths and only the peel cost varies.
 */
function expensiveRun(outerLen: number, layers: number): string {
  const seed = Math.max(24, Math.floor((outerLen * Math.pow(0.75, layers) * 3) / 4));
  let s = randomBytes(seed).toString("base64").replace(/[+/=]/g, "A");
  for (let i = 0; i < layers; i++) s = Buffer.from(s, "utf8").toString("base64");
  return s;
}

describe("scanForSecrets — budget starvation (per-run bound + no break)", () => {
  const secretBlob = Buffer.from(ANT_KEY, "utf8").toString("base64");

  // The core regression: the credential sits AFTER the expensive run, so it is
  // reached only if the expensive run cannot eat the whole scan budget.
  // Filler sized so the peel would exhaust the WHOLE scan budget without a
  // per-run bound. Measured: with the per-run bound removed OR `break` restored,
  // these exact documents flip to MISSED in all three passes at every depth >= 2.
  const FILLER_LEN = 96_000;
  for (const layers of [1, 2, 8, 24]) {
    it(`finds a base64-wrapped key after an expensive ${layers}-layer run`, () => {
      const doc = `filler=${expensiveRun(FILLER_LEN, layers)} key=${secretBlob}`;
      const r = scanForSecrets(doc);
      expect(r.clean).toBe(false);
      expect(r.matches.some((m) => m.type === "obfuscated")).toBe(true);
      // And the span must point at the credential blob, not the filler.
      expect(redactSecrets(doc)).not.toContain(secretBlob);
    }, 60_000);
  }

  // The known-value pass (drives the egress BLOCK) was defeated by the same
  // starvation — it has its own copy of the run loop.
  it("known-value pass still fires after an expensive run", () => {
    const KNOWN = "kv" + randomBytes(24).toString("hex");
    registerRedactedSecretValue(KNOWN);
    try {
      const wrapped = Buffer.from(KNOWN, "utf8").toString("base64");
      const doc = `filler=${expensiveRun(96_000, 4)} v=${wrapped}`;
      const r = scanForSecrets(doc);
      expect(r.matches.some((m) => m.type === "known-secret-value")).toBe(true);
    } finally {
      unregisterRedactedSecretValue(KNOWN);
  ...
  // The taint-overlap pass (decodedPayloadViews) is the third consumer.
  it("decodedPayloadViews still surfaces a later run's decoded bytes", () => {
    const doc = `filler=${expensiveRun(96_000, 4)} key=${secretBlob}`;
    expect(decodedPayloadViews(doc).some((v) => v.includes(ANT_KEY))).toBe(true);
  }, 60_000);

  // A scan that hits the scan-wide ceiling must be OBSERVABLE, never a silent
  // clean. `clean` is fail-closed so existing callers that read it as proof of
  // safety cannot be handed unscanned bytes.
  it("reports truncated (and never clean) when the scan ceiling is reached", () => {
    const many = Array.from({ length: 64 }, () => `f=${expensiveRun(20_000, 3)}`).join(" ");
    const r = scanForSecrets(many);
    expect(r.truncated).toBe(true);
    expect(r.clean).toBe(false);
  }, 120_000);

  it("ordinary text is neither truncated nor dirty", () => {
    const r = scanForSecrets("the quarterly report is attached, please review");
    expect(r.truncated).toBe(false);
    expect(r.clean).toBe(true);
  });
});

// ── Attribution span: a match may not claim the whole document ────────────────
//
// PERCENT_RUN_RE is `(?:%[0-9a-fA-F]{2}|[^\s%]){8,}` — its `[^\s%]` alternative
// means a "run" is any whitespace-free stretch containing an escape, i.e. an
// entire minified JSON response. Attributing the match to the whole run made
// redactSecrets overwrite the document instead of the credential.
describe("scanForSecrets — percent-run attribution span", () => {
  it("redacts the credential, not the whole whitespace-free document", () => {
    // Hand-escaped: encodeURIComponent leaves this key untouched (it is already
    // URL-safe), and an unescaped key is found by the RAW pass, which would not
    // exercise percent attribution at all.
    const enc = ANT_KEY.replace(/a/g, "%61").replace(/-/g, "%2D");
    expect(enc).not.toContain(ANT_KEY);
    const before = `{"a":"x1","b":"y2","token":"${enc}","c":"z3","d":"w4","e":"v5"}`;
    expect(before).not.toMatch(/\s/);
    const after = redactSecrets(before);
    expect(after).not.toContain(ANT_KEY);
    expect(after).not.toContain(enc);
    // Everything around the credential survives.
    expect(after).toContain('{"a":"x1","b":"y2","token":"');
    expect(after).toContain('","c":"z3","d":"w4","e":"v5"}');
    expect(after.length).toBeGreaterThan(before.length - enc.length);
  });

  it("attributes a nested percent(base64(key)) run to the inner blob", () => {
    const inner = Buffer.from(ANT_KEY, "utf8").toString("base64");
    const before = `{"t":"%20${inner}","tail":"bbbbbbbbbbbbbbbb"}`;
    expect(before).not.toMatch(/\s/);
    const after = redactSecrets(before);
    expect(after).not.toContain(inner);
    expect(after).toContain('{"t":"%20');
    expect(after).toContain('"tail":"bbbbbbbbbbbbbbbb"}');
  });

  it("a base64 blob is still attributed to the whole blob", () => {
    const blob = Buffer.from(ANT_KEY, "utf8").toString("base64");
    const text = `note=${blob} tail`;
    const r = scanForSecrets(text);
    const m = r.matches.find((x) => x.type === "obfuscated")!;
    expect(text.slice(m.startIndex, m.endIndex)).toBe(blob);
  });
});
