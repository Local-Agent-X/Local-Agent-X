...
const SAX_REPO_ROOT = resolvePath(new URL("..", import.meta.url).pathname.replace(/^\//, "").replace(/\/$/, ""));
...
async function collectSubtreeRules(scopeHint: string): Promise<string> {
  try {
    ...
    const resolved = scopeHint
      ? (isAbsolute(scopeHint) ? scopeHint : resolve(SAX_REPO_ROOT, scopeHint))
      : SAX_REPO_ROOT;
    ...
    while (true) {
      dirs.push(cur);
      if (cur === SAX_REPO_ROOT || !cur.startsWith(SAX_REPO_ROOT)) break;
      const parent = dirname(cur);
      if (parent === cur) break;
      cur = parent;
    }
    // Ensure repo root is included
    if (!dirs.includes(SAX_REPO_ROOT)) dirs.push(SAX_REPO_ROOT);
    // Collect AGENTS.md contents (root first, then deeper subtrees)
    const parts: string[] = [];
    for (const d of dirs.reverse()) {
      const p = join(d, "AGENTS.md");
      if (existsSync(p)) {
        const rel = relative(SAX_REPO_ROOT, p).replace(/\\/g, "/") || "AGENTS.md";
