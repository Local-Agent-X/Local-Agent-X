  ...
  eventBus.on("handler:agent-run", async (data: unknown) => {
    ...
    try {
      ...
      const agentResult = await enqueue("agent", () => runAgent(task, agentSession.messages, {
        apiKey, model, provider: provider as AgentOptions["provider"], systemPrompt: (systemPrompt || `You are a ${role} agent. Complete the task. STOP if login is needed or after 3 failed attempts. End with a summary.`) + `\n\nEXECUTION RULES:\n- Do the simplest thing first. Unzip before parsing. Read before transforming. One step at a time.\n- For large files (>1MB): use python -c or write a script to process them. Do NOT use the read tool line-by-line — one python/bash command can do what 1000 read calls would take.\n- For JSON files: use python or jq to extract summaries, counts, and key fields — never try to read raw JSON line by line.\n- Bash commands time out at 120s. Break large operations into chunks that each finish within 60s.\n- If a command fails or times out, try a simpler approach. Don't retry the same thing.\n- Save intermediate results to workspace/ so progress isn't lost if you hit limits.\n- You have a maximum of 25 tool calls. Budget them wisely — each call should accomplish significant work.\n` + identityBlock + parentContext + briefing + worktreeBlock,
