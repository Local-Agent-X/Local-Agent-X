...
const LEGACY_DEFAULT_DECISIONS: Record<string, ReadonlyArray<ToolPolicyRule["decision"]>> = {
  ...
  // NOTE: browser.emulate / browser.layout_report add NO entry here. This list
  // is only for defaults whose DECISION changed in code (so a legacy snapshot
  // rule can be recognised as stale and refreshed). A brand-new action that
  // ships with no rule of its own has no prior decision to retire.
};
