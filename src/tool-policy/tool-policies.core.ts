...
export const TOOL_POLICIES_CORE: Record<string, ToolPolicyEntry> = {
  ...
  transcribe_media:   { kernel: "file", risk: "safe", offBoxFetch: true, pathArgs: [{ arg: "path", action: "read" }], rules: [{ id: "allow-transcribe-media", decision: "allow", reason: "Transcribes a local recording (path-checked read); ships audio off-box when a CLOUD stt provider is configured", priority: 50 }] },
  read_video_frames:  { kernel: "file", risk: "safe", offBoxFetch: true, pathArgs: [{ arg: "path", action: "read" }], rules: [{ id: "allow-read-video-frames", decision: "allow", reason: "Samples frames from a local video (path-checked read); base64-ships them off-box to the vision API", priority: 50 }] },
  ...
  send_file:   { kernel: "file", risk: "safe", offBoxFetch: true, pathArgs: [{ arg: "path", action: "read" }], rules: [{ id: "allow-send-file", decision: "allow", reason: "Stages a local document for the user's own device to fetch (path-checked read)", priority: 50 }] },
