...
export const BROWSER_TOOL_DESCRIPTION =
  ...
  "- emulate: Render as a DIFFERENT DEVICE for the rest of the session — viewport size, deviceScaleFactor, isMobile/hasTouch, and USER AGENT. Pass device='iphone'|'android'|'ipad' (or 'desktop' to clear), or viewport_width + viewport_height (plus optional user_agent / is_mobile / has_touch / device_scale_factor). USE THIS BEFORE INVESTIGATING ANY MOBILE-ONLY BUG: many sites serve phones a DIFFERENT HTML document keyed on the user agent, so a desktop rendering can look perfectly clean while the reported defect is right there on mobile. Emulation opens a FRESH isolated context (the current URL is re-opened; cookies/logins do NOT carry over), and is unavailable on the in-app browser because that window is the user's own.\n" +
  "- layout_report: ONE call that measures the CURRENT page's layout — do not hand-roll this with evaluate and never write a scratch HTML file to test it. Returns: horizontal overflow (documentElement.scrollWidth vs clientWidth), every element whose rect extends past the viewport with a usable selector and its rect (worst 20 listed, with the true total), which @media queries currently match, the computed background colour of html/body/the page canvas, and the geometry of every position:fixed / position:sticky element. That combination is what identifies stray bars, gaps, and strips above/below fixed navigation. Read-only: it mutates nothing on the page.\n" +
