...
export const BROWSER_TOOL_DESCRIPTION =
  ...
  "- layout_report: ONE read-only call that returns RAW layout data for the CURRENT page as JSON — do not hand-roll this with evaluate and never write a scratch HTML file to test it. The JSON holds: document overflow (scrollWidth vs clientWidth), every element whose rect extends past the viewport with a selector and rect (worst 20 listed, true total given), which @media queries currently match, the computed html/body/canvas background colours, and the geometry of every position:fixed / position:sticky element. It states NO verdict; you must read the completeness flags yourself: cssWalkIncomplete, elementScanIncomplete, scanTruncated, unreadableStyleSheets and the rest — wherever one is set, the counts it covers are LOWER BOUNDS, and a zero there is not evidence of absence. knownGaps lists what the probe can never see (@container queries, closed shadow roots, the inside of open shadow roots and iframes). Pair with emulate to measure the mobile rendering.\n" +
