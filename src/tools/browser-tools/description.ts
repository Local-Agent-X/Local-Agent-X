...
export const BROWSER_TOOL_DESCRIPTION =
  ...
  "- emulate: Render as a DIFFERENT DEVICE for the rest of the session — viewport size, deviceScaleFactor, isMobile/hasTouch, and USER AGENT. Pass device='iphone'|'android'|'ipad' (or 'desktop' to clear), or viewport_width + viewport_height (plus optional user_agent / is_mobile / has_touch / device_scale_factor). USE THIS BEFORE INVESTIGATING ANY MOBILE-ONLY BUG: many sites serve phones a DIFFERENT HTML document keyed on the user agent, so a desktop rendering can look perfectly clean while the reported defect is right there on mobile. Then use screenshot/snapshot to see the page as that device does. Emulation opens a FRESH isolated context (the current URL is re-opened; cookies/logins do NOT carry over). On the in-app browser that context is PRIVATE (headless unless an earlier external-Chrome fallback had already started this session's Chrome visible) and the user's own window is left untouched — but this session's page actions (navigate/snapshot/screenshot/extract/evaluate/layout_report/click/fill/tabs, and the secret fill/capture tools) then run against the emulated context until you call emulate device='desktop' to go back, and read_console/read_network/read_response are unavailable while emulating. TWO CONSEQUENCES THAT MISLEAD IF YOU DO NOT KNOW THEM: (1) the emulated context has NO cookies and NO logins, so a login-gated page renders LOGGED OUT there — a mobile-vs-desktop comparison on an authenticated page is then comparing a logged-out mobile document against a logged-in desktop one, which is a different page and not a mobile layout defect; (2) while emulating, 'tabs' lists only the emulated context's own tabs — the [user tab] rows disappear and 'switch_tab' can no longer reach the tabs the user has open, so the usual escape hatch for a login-gated page (the user is already signed in there) is unavailable until emulation is cleared. After emulating, layout_report is the call that shows WHICH element produces the mobile layout.\n" +
  "- layout_report: REACH FOR THIS FIRST whenever the question is about how a page LOOKS — 'why does this page look wrong', 'what is that grey strip/bar', 'what is causing the horizontal scroll', 'which @media rule is matching at this width', 'does our page match the original' — instead of hand-rolling probes with evaluate or writing a scratch script. ONE read-only call that returns RAW layout data for the CURRENT page as JSON — do not hand-roll this with evaluate and do not write a scratch HTML file to test it. The JSON holds: document overflow (scrollWidth vs clientWidth), scanned elements whose rect extends past the viewport with a selector and rect (worst 20 listed; overflowingElementsTotal counts the scanned ones), which @media conditions currently match (matchingMediaQueriesTotal counts distinct full conditions; the listed text may be cut), the computed html/body/canvas background colours, and the geometry of scanned position:fixed / position:sticky elements. Rules: an element whose OWN computed display is none or visibility is hidden is not measured (hiddenElementsSkipped — head elements such as script/style/meta/link/title count here); an element whose rect has 0 width or 0 height is not measured (zeroSizeElementsSkipped — this is where the descendants of a display:none ancestor land, since their own computed display is not none); any positive rounded overflow past the right OR left viewport edge is listed. Stylesheets that are disabled, or whose own media condition (link media=, @import ... media) does not currently match, are not walked (disabledSheetsSkipped, sheetsSkippedByMedia). Page strings are cut to fixed sizes: url and userAgent (urlTruncated, userAgentTruncated), labels, media text, and selectors — a row with selectorTruncated: true holds a cut selector that is NOT usable for querySelector; use its rect and text instead. A page value that is not a finite number is reported as null and named in nonNumericFields (that name list is capped — nonNumericFieldsTruncated says more were dropped). The document is JSON under a size budget: when list rows had to be dropped to fit it sets listsTrimmedForSize and the *Listed counts say how many remain, when even the scalars did not fit you get {reportTooLarge, bytesBeforeFallback, url, knownGaps}, when a page global threw you get {reportFailed, error, knownGaps}, and a page that swapped out a JS builtin before the probe started can still produce output truncated to '[Truncated at 8000 chars]' rather than JSON. The untrusted-content wrapper may substitute [REDACTED_SECRET] for a registered secret value inside a string; apart from that it leaves the document's shape and counts as the page produced them. It states NO verdict; read the completeness flags yourself: cssWalkIncomplete, elementScanIncomplete, scanTruncated, listsTrimmedForSize, unreadableStyleSheets and the rest — wherever one is set, the counts it covers are LOWER BOUNDS, and a zero there is not evidence of absence. knownGaps lists what the probe does not see (@container queries, closed shadow roots, the inside of open shadow roots and iframes). Pair with emulate to measure the mobile rendering.\n" +
...
export const BROWSER_TOOL_PARAMETERS = {
  ...
  properties: {
    action: {
      type: "string",
      enum: ["navigate", "new_tab", "snapshot", "click", "click_text", "fill", "select", "extract", "screenshot", "evaluate", "act", "observe", "emulate", "layout_report", "scroll", "tabs", "switch_tab", "close_tab", "info", "read_console", "read_network", "read_response", "downloads", "release_download", "dialog_accept", "dialog_dismiss", "history", "bookmark_add", "bookmarks", "close"],
