 ...
 *
 * The tool result is the compact JSON string produced by browser/layout-report.ts inside the untrusted-content
 * wrapper, and nothing else: no preamble, no summary, no flags sentence, no emulation profile. The page names
 ...
 *
 * What layout-report.test.ts pins, per test: the result text is byte-for-byte
 * wrapExternalContent(<what evaluate returned>, "browser.layout_report") for a clean report, a truncated report,
 * a non-JSON string and a session with an emulation profile installed; the constant clears the evaluate
 * blocklist and the handler refuses if it ever stops clearing it; one evaluate call and no mutating backend
 * method.
 *
