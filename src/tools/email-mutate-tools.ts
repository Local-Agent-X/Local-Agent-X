    ...
    return withSession(cfg, async (session): Promise<ToolResult> => {
      ...
      if (cursor) {
        ...
        if (commit?.more) {
          ...
          payload.remaining = 0;
          // `commit` is null only if the entry vanished under us — another call
          // on the same cursor finished the sweep and deleted the store entry.
          // THIS page's own shortfall is then the wrong number to reason from:
          // it can be zero while the vanished entry still carried residue from
          // an earlier page, and zero prints the completion claim. When the
          // sweep-level total is unavailable the honest state is UNKNOWN, not
          // complete, so we never take the terminal branch.
          const residue = commit ? commit.unconfirmed : unconfirmed;
          payload.unconfirmed = residue;
          note += commit && residue === 0
