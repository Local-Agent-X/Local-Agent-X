...
export function fullStackRuleLines(appName: string, appDir: string): string[] {
  return [
    ...
    `- Build a real backend under ${appDir}/server (Node + Express is fine). For storage, PREFER Node's built-in SQLite — import { DatabaseSync } from "node:sqlite", persist to ${appDir}/server/data.db. It needs NO install and NO native build, so it can never fail to compile.`,
    `- If you instead use a native module (better-sqlite3 is fine and well-supported), depend on its LATEST version ("better-sqlite3": "latest") so npm fetches a prebuilt binary matching this machine's Node. NEVER pin an old version like "^9.x" — old native modules have no prebuilt for a current Node, fall back to a source compile, and fail, leaving the app with a dead backend.`,
    `- Start it with ONE call: app_serve_backend({ app_id: "${appName}", command: "cd server && npm install && npm run dev", port: <the port your server listens on> }). The command runs from the APP ROOT (${appDir}), so cd into the server folder first. app_serve_backend starts the backend, keeps it alive (LAX restarts it when the app is opened and stops it when the app is deleted), and wires the connector for you. Do NOT start a dev server with bash — bash blocks the turn and times out.`,
