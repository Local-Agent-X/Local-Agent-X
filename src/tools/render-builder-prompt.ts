...
export function fullStackRuleLines(appName: string, appDir: string): string[] {
  return [
    ...
    `- Start it with ONE call: app_serve_backend({ app_id: "${appName}", command: "npm install && npm run dev", port: <the port your server listens on> }). That starts the backend, keeps it alive (LAX restarts it when the app is opened and stops it when the app is deleted), and wires the connector for you. Do NOT start a dev server with bash — bash blocks the turn and times out.`,
