...
for /f "tokens=*" %%t in ('node -e "try{const f=require('fs'),p=require('path'),o=require('os');const c=JSON.parse(f.readFileSync(p.join(o.homedir(),'.lax','config.json'),'utf-8'));console.log(c.authToken)}catch{try{const s=f.readFileSync(p.join(o.homedir(),'.lax','settings.json'),'utf-8');console.log(JSON.parse(s).authToken||'')}catch{}}" 2^>nul') do set "TOKEN=%%t"
