...
import {
  ...
  statePath,
  STATE_FILENAME,
} from "../src/auto-build/failure-recovery.js";

let dir: string;
beforeEach(() => { dir = mkdtempSync(join(tmpdir(), "auto-build-recovery-test-")); });
...
describe("state file is named the expected filename", () => {
  it("uses .lax-build-history.json in project_dir", () => {
    expect(STATE_FILENAME).toBe(".lax-build-history.json");
