 ...
 *   SEAM 2 — THE COUNTER. `middleware_fired` is minted (guard-fire.ts) on the
 *            nudge path AND on the abort path, and is ABSENT when a guard
 *            stands down.
 *
 * The gate seams this file also covered (broad-sweep-nudge / cleanup-verify /
 * codebase-advice standing down on a harness-stamped task, and the reason
 * vocabulary the retraction dispatch keyed off) went out with those guards on
 * 2026-09-16: they judged the model's wording. What remains is the provenance
 * that still gates the instruction ledger, and the counter.
 *
